...
export const makeCARs = (config: any, snowflake?: string) => {
    ...
    return async (msg: Message) => {
        ...
        // don't bother parsing CARs if content is blank
        if (content === '') {
            return false;
        }

        // return if channel is blacklisted
        // @ts-ignore
        if (msg.channel.type === 'text' && blacklist.includes(msg.channel.name)) {
            return false;
        }
...
const processCARS = async (message: Message, callAndResponses: CARData[] = [], {reply = false, verbose = false, snowflake = undefined}: processCarsOpts) => {
    ...
    const mentioned = mentionedUsers.some((x: User) => x.id === snowflake);
    const messageContent = normalizeStr(message.content, {
        preserveWhiteSpace: true,
        preserveUrl: true,
        mentions: mentionedUsers.filter((x: User) => x.id !== snowflake)
    });
    ...
    for (const cr of callAndResponses) {
        ...
        if (response.length > 0) {
            ...
            if (typeof responses === 'string') { // we're looking for a nickname
                const foundCar = callAndResponses.find(x => x.nickname === response);
